...
> **来源：** 蘑菇物语论坛 https://moguwuyu.com/d/157-让北斗冒险岛同时支持拍卖行脚本和进入拍卖界面
> **作者：** leevccc（2026年2月13日）
> **整理时间：** 2026-04-26
...
**目标：** 在拍卖行脚本里增加一个菜单项，通过这个菜单进入 MapleStory 原始的拍卖界面。

**背景：** 原本点击拍卖按钮是进入拍卖界面的，但北斗做了处理，改为呼出拍卖行脚本（`9900001.js`）。我们需要找到原始进入拍卖界面的方法，并让脚本也能调用。

---

## 分析过程

### 1. 在服务端搜索 9900001（范围限定在 Java 文件）
...
**关键发现：** 如果配置项中没有启用拍卖（`use_mts = false`），则打开拍卖行脚本。在这段之后的代码就是原来呼出拍卖界面的代码。
...
其中 `enterMTS` 里的代码是从 `EnterMTSHandler` 的 `handler` 内复制过来的，另外两个方法（`getNotYetSold`、`getTransfer`）也是从 `EnterMTSHandler` 中复制过来的。

### 修改 2：简化 EnterMTSHandler

**文件：** `gms-server/src/main/java/org/gms/net/server/channel/handlers/EnterMTSHandler.java`
...
**说明：** 把 `EnterMTSHandler` 中原有的 `enterMTS`、`getNotYetSold`、`getTransfer` 三个方法删掉，改为调用 `Character.enterMTS()`。

### 修改 3：编辑拍卖行脚本 9900001.js

**文件：** `gms-server/scripts-zh-CN/npc/9900001.js`
...
```javascript
case 5:  
    cm.getPlayer().enterMTS(cm.getClient());  
    break;
```
...
点击拍卖按钮 → 呼出拍卖行脚本 → 脚本里多了个「拍卖」菜单 → 选择后进入原始拍卖界面。
...
| 文件 | 操作 | 说明 |
|------|------|------|
| `Character.java` | 新增方法 | 添加 `enterMTS()`、`getNotYetSold()`、`getTransfer()` |
| `EnterMTSHandler.java` | 简化 | 删除三个方法，改为调用 `c.getPlayer().enterMTS(c)` |
| `9900001.js` | 新增菜单 | 添加「拍卖」菜单项（case 5），调用 `cm.getPlayer().enterMTS()` |

---

*作者原话：这个教程只是一个示例，教你怎么吃饱，至于怎么吃好，你自己琢磨吧。*
